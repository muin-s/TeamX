...
exports.jobsByRecruiter = async (req, res, next) => {
  try {
    ...
    const query = { recruiter: new ObjectID(req.user._id) };
    const jobs = await Job.find(query);

    for (const job of jobs) {
      const job_id = job['_id'];
      const application_count = await sql.query(`SELECT COUNT(*) as count FROM job_application WHERE job_id = '${job_id}'`);
      const save_count = await sql.query(`SELECT COUNT(*) as count FROM saved_job WHERE job_id = '${job_id}'`);
      const convertedJobJSON = { ...JSON.parse(JSON.stringify(job)), application_count: application_count[0].count, save_count: save_count[0].count };
  ...
  try {
    ...
    for (const element of jobs) {
      if (skills.length > 0 && element.skills) {
        passesCriteria = skills.some(skill => element.skills.includes(skill));
      }

      if (passesCriteria && response.payLoad.length < 12) {
        response.payLoad.push(element);
      }
    }

    if (response.payLoad.length < 12) {
      const lat = user.address?.coordinates?.latitude || null;
      const long = user.address?.coordinates?.longitude || null;

      for (const element of jobs) {
        passesCriteria = lat && long;
    ...
    if (response.payLoad.length < 12) {
      for (let index = 0; index < Math.min(jobs.length, 10); index++) {
        response.payLoad.push(jobs[index]);
      }
    }
